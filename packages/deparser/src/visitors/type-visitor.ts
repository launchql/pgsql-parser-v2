...
export class TypeVisitor extends BaseVisitor {
  ...
  visit(node: Node, context: DeparserContext = {}): string {
    ...
    switch (nodeType) {
      case 'TypeName':
        return this.visitTypeName(nodeData, context);
      case 'Alias':
        return this.visitAlias(nodeData, context);
      case 'RangeVar':
        return this.visitRangeVar(nodeData, context);
      default:
        throw new Error(`Type visitor does not handle node type: ${nodeType}`);
    }
  }

  private visitTypeName(node: TypeName, context: DeparserContext): string {
  ...
  private visitAlias(node: Alias, context: DeparserContext): string {
  ...
  private visitRangeVar(node: RangeVar, context: DeparserContext): string {
    const output: string[] = [];
