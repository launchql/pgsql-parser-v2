...
export class StatementVisitor extends BaseVisitor {
  ...
  visit(node: Node, context: DeparserContext = {}): string {
    ...
    switch (nodeType) {
      case 'RawStmt':
        return this.visitRawStmt(nodeData, context);
      case 'SelectStmt':
        return this.visitSelectStmt(nodeData, context);
      case 'InsertStmt':
        return this.visitInsertStmt(nodeData, context);
      case 'UpdateStmt':
        return this.visitUpdateStmt(nodeData, context);
      case 'DeleteStmt':
        return this.visitDeleteStmt(nodeData, context);
      case 'WithClause':
        return this.visitWithClause(nodeData, context);
      case 'ResTarget':
        return this.visitResTarget(nodeData, context);
      default:
        throw new Error(`Statement visitor does not handle node type: ${nodeType}`);
    }
  }

  private visitRawStmt(node: RawStmt, context: DeparserContext): string {
    if (node.stmt_len) {
  ...
  private visitSelectStmt(node: SelectStmt, context: DeparserContext): string {
    const output: string[] = [];
    ...
    if (node && (!node.op || (node.op as string) === 'SETOP_NONE')) {
      ...
      output.push(this.formatter.parens(this.visitSelectStmt(node.larg as SelectStmt, context)));
      ...
      if (node) {
        output.push(this.formatter.parens(this.visitSelectStmt(node.rarg as SelectStmt, context)));
      }
    }
  ...
  private visitInsertStmt(node: InsertStmt, context: DeparserContext): string {
    ...
    if (node.onConflictClause) {
      ...
      switch (clause.action as string) {
        ...
        case 'ONCONFLICT_UPDATE':
          output.push('DO');
          output.push(this.visitUpdateStmt(clause as any, context));
          break;
        default:
  ...
  private visitUpdateStmt(node: UpdateStmt, context: DeparserContext): string {
    const output: string[] = [];
  ...
  private visitDeleteStmt(node: DeleteStmt, context: DeparserContext): string {
    const output: string[] = [];
  ...
  private visitWithClause(node: WithClause, context: DeparserContext): string {
  ...
  private visitResTarget(node: ResTarget, context: DeparserContext): string {
    const output: string[] = [];
